...
// Campos aceitos no arquivo, mapeando variações de nome para o padrão interno
...
importInput.addEventListener("change", async (e) => {
    ...
    try {
        ...
        if (invalid.length > 0) {
            const lines = invalide.map((r) => `Lina ${r.lines}: ${r.reason}`).json("\n");
...
// Valida cada linha e separa válidas e invalidas

const validateRows = (rows) => {
    const valid = [];
    const invalid = [];

    rows.array.forEach((row, index) => {
